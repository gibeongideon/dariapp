...
class WheelSpin(TimeStamp):
    ...
    def total_bet_amount_per_marktinstance(self):
        try:
            total_amount = Stake.objects.filter(
                market_id=self.id, bet_on_real_account=True
            ).aggregate(bet_amount=Sum("amount"))
    ...
    def market_stake_amount(self, select_id):
        ...
        try:
            total_amount = (
                Stake.objects.filter(market_id=self.id, bet_on_real_account=True)
                .filter(marketselection_id=select_id)
                .aggregate(bet_amount=Sum("amount"))
            )
...
class Stake(TimeStamp):
    ...
    def bet_status(self):
        try:
            try:
                ...
                if (
                    self.marketselection_id
                    == OutCome.objects.get(market_id=self.market.id).result
                ):
                    return "wind"
                return "losed"
            except Exception as e:
    ...
    def unspinned(cls, user_id):
        return [
            obj.id
            for obj in cls.objects.filter(user=user_id, market=None, spinned=False)
        ]
...
class OutCome(TimeStamp):
    ...
    # DARU
    @staticmethod
    def per_return_relief(
        all_gain, userstake, all_lose_stake, per_to_return
    ):  ##CRITICAL FUCTION/MUST WORK PROPERLY
        try:
        ...
        if this_user_stak_obj.bet_on_real_account == True:  # REAL
            if this_user_stak_obj.market is not None:  # daru
                if this_user_stak_obj.marketselection_id == self.result:
                    try:
                        win_amount, ref_credit, rem_credit = self.update_values(
                            this_user_stak_obj
                        )
                        trans_type = "win"
                ...
                if self.result == 5:  ###
                    ...
                    all_amount = float(win_amount * set_up.big_win_multiplier) + float(
                        this_user_stak_obj.amount
                    )
                    self.update_user_real_account(user_id, all_amount)
    ...
    def run_account_update(self):
        if self.market is not None:
            try:
                all_stakes_in_this_market = Stake.objects.filter(
                    market=self.market
                ).all()  # R
    ...
    def update_db_records(self):
        try:
            ...
            WheelSpin.objects.filter(id=self.market_id).update(
                receive_results=True
            )  # self.market.update(closed=True) or self.market.closed=True DOESN'T WORK
